...
useEffect(()=>{
    console.log('broooo')
    axios.get(`${API_URL}boroughs/mutualaids/?borough_id=1`).then((response) => {
...
console.log("in delete")
    ...
    axios.delete(`${API_URL}boroughs/mutualaids/?borough_id=1&id=${oneaid.id}`).then((response) => {
...
const onMutualAidClick = (mutualaid) => {
    ...
    axios.get(`${API_URL}boroughs/mutualaids/?borough_id=1&${mutualaid.id}`).then((response) => {
    ...
    const onOneAidClick = (oneaid) => {
        ...
        axios.get(`${API_URL}boroughs/mutualaids/?borough_id=1&${oneaidData.id}`).then((response) => {
        setOneAidData(oneaid);
        // setMutualAidData([...mutualaidData, ...oneaid])
        }).catch((error) => {
            console.log('Error: Couldn\'t get all aid', error)
